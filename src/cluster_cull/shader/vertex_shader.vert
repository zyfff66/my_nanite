...
FrameContext get_frame_context()
{
    ...
    for(int i=0;i<4;i++)
    {
        ...
        p.y=uintBitsToFloat(buffers[idx].data[i*4+48+1]);
        p.z=uintBitsToFloat(buffers[idx].data[i*4+48+2]);
        p.w=uintBitsToFloat(buffers[idx].data[i*4+48+3]);
        context.vp_mat2[i]=p;
    }
...
uvec2 get_visible_cluster(uint cluster_id)
{
    uint idx=pc.swapchain_idx+1+num_swapchain_image();
    return uvec2(buffers[idx].data[cluster_id*2],buffers[idx].data[cluster_id*2+1]);
}
...
vec3 get_position(Cluster cluster,uint index)
{
    uint tri_id=index/3;
    uint idx=1+3*num_swapchain_image();
    uint tri=buffers[idx].data[tri_id+cluster.t_data_ofs];
    uint v_idx=((tri>>(index%3*8))&255);

    vec3 p;
    p.x=uintBitsToFloat(buffers[idx].data[v_idx*3+cluster.v_data_ofs]);
    p.y=uintBitsToFloat(buffers[idx].data[v_idx*3+cluster.v_data_ofs+1]);
    p.z=uintBitsToFloat(buffers[idx].data[v_idx*3+cluster.v_data_ofs+2]);
    return p;
}
...
void main()
{
    uvec2 visible_cluster=get_visible_cluster(gl_InstanceIndex);
    uint cluster_id=visible_cluster.x;
    uint instance_id=visible_cluster.y;
    ...
    if(context.view_mode==0)
        color=to_color(tri_id);
    else if(context.view_mode==1)
        color=to_color(cluster_id);
    else if(context.view_mode==2)
        color=to_color(cluster.group_id);
    else if(context.view_mode==3)
        color=to_color(cluster.mip_level);
