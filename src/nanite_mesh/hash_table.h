...
inline u32 murmur32(u32 hash,u32 elememt){
	elememt*=0xcc9e2d51;
...
inline u32 murmur_finalize32(u32 hash){
	hash^=hash>>16;
	hash*=0x85ebca6b;
	hash^=hash>>13;
	hash*=0xc2b2ae35;
...
inline u32 round_up_to_power_of_two(u32 x){
    while(x&(x-1)) x^=(x&-x);
    return x;
}

inline u32 round_up_to_power_of_two2(u32 x){
    if(x&(x-1)){
        while(x&(x-1)) x^=(x&-x);
    }
    return x==0?1:(x<<1);
...
public:
    ...
    void add(u32 key,u32 idx){
        if(idx>=index_size){
            resize_index(round_up_to_power_of_two2(idx+1));
        }
        key&=hash_mask;
